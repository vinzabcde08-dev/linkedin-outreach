...
export default function ProfileSetup({ onProfileSaved }) {
  ...
  async function handleParseProfile() {
    ...
    try {
      ...
      setParseError(e instanceof SyntaxError
        ? 'Claude returned unexpected data — please try again.'
        : e.message)
    } finally {
  ...
  function handleApplyParsed() {
    ...
    // Merge parsed fields — only overwrite if value is non-empty
    const merged = { ...profile }
    const stringFields = ['fullName','preferredName','title','company','location','email','phone',
                          'websiteUrl','portfolioUrl','linkedinUrl','bio','toneOfVoice','targetClients','linkedinGoal']
    stringFields.forEach(f => {
      if (parsedPreview[f] && parsedPreview[f].trim()) merged[f] = parsedPreview[f].trim()
    })
        ...
        <div className="fade-in space-y-4">
            ...
            <div className="flex gap-3 items-start">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Paste your resume, LinkedIn bio, "about me" text — anything that describes you. Claude will read it and fill in your entire profile automatically.
                </p>
                <p className="text-xs text-blue-600 mt-2 font-medium">
                  Works with: resume, LinkedIn summary, website bio, any freeform text about yourself
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <p className="text-xs text-gray-400 mb-3">
              Copy and paste from anywhere — Word doc, Google Doc, LinkedIn, your website. The more detail, the better.
            </p>
            <textarea
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <button
                ...
                disabled={parseLoading || pasteText.trim().length < 50}
                className="btn-primary"
              >
                {parseLoading ? (
                  <><span className="spinner border-white border-t-transparent" /> Reading your text...</>
                ) : (
                  '🪄 Parse & Fill Profile'
                )}
              </button>
          ...
          {parsedPreview && (
            <div className="card border-green-200 bg-green-50 fade-in">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={handleApplyParsed} className="btn-primary text-sm">
                  ✅ Apply to Profile
                </button>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {parsedPreview.services?.length > 0 && (
                    <div className="bg-white rounded-lg p-3 border border-green-100">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {parsedPreview.services.map((s, i) => (
                          <span key={i} className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full text-[10px]">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {parsedPreview.tools?.length > 0 && (
                    <div className="bg-white rounded-lg p-3 border border-green-100">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {parsedPreview.tools.map((t, i) => (
                          <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full text-[10px]">{t}</span>
                        ))}
                      </div>
                    </div>
                  )}
                ...
                {parsedPreview.achievements?.length > 0 && (
                  <div className="bg-white rounded-lg p-3 border border-green-100">
                    ...
                    <ul className="space-y-1">
                      {parsedPreview.achievements.map((a, i) => (
                        <li key={i} className="text-gray-700 flex gap-2"><span className="text-green-500 flex-shrink-0">✓</span>{a}</li>
                      ))}
                    </ul>
                  </div>
                )}
              ...
              <div className="mt-4 pt-3 border-t border-green-200 flex items-center justify-between">
                <p className="text-xs text-green-700">
                  After applying, go to each tab to review and fine-tune any details.
                </p>
                <button onClick={handleApplyParsed} className="btn-primary text-sm">
                  ✅ Apply to Profile
                </button>
