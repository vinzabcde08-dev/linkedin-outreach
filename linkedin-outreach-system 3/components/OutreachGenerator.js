...
export default function OutreachGenerator({ prospect }) {
  ...
  function extractSection(text, sectionKey) {
    const patterns = {
      connection: /##\s*[✉️🔗]?\s*1\.\s*CONNECTION REQUEST[\s\S]*?(?=##\s*[💬🔁]|$)/i,
      firstDm:    /##\s*[💬]?\s*2\.\s*FIRST DM[\s\S]*?(?=##\s*[🔁✉️]|$)/i,
      followUp1:  /##\s*[🔁]?\s*3\.\s*FOLLOW-UP 1[\s\S]*?(?=##\s*[🔁📌]|$)/i,
      followUp2:  /##\s*[🔁]?\s*4\.\s*FOLLOW-UP 2[\s\S]*?(?=##\s*[🔁📌]|$)/i,
      followUp3:  /##\s*[🔁]?\s*5\.\s*FOLLOW-UP 3[\s\S]*?(?=##\s*[📌]|$)/i,
    }
