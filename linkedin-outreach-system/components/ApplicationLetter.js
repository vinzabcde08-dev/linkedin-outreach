...
export default function ApplicationLetter() {
  ...
  // Auto-populate resume link from profile on mount
  useEffect(() => {
    const p = getProfile()
    const link = p?.portfolioUrl || p?.websiteUrl || ''
    if (link) setResumeLink(link)
    try {
      const savedNotes = localStorage.getItem('los_appletter_notes')
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      let parsed = null
      try {
        const parseText = await callClaude('parseJobPost', { jobPost }, profile)
        parsed = safeParseJson(parseText)
      } catch {}
      if (!parsed) throw new Error('Could not parse the job post. Try again.')
      setParsedJob(parsed)

      // Research the employer using the extracted company name
      const companyName = parsed.companyName || parsed.employerProfile?.name || ''
      if (companyName) {
        try {
          const researchText = await callClaude('researchEmployer', { companyName, jobDescription: jobPost }, profile)
          try { setEmployer(safeParseJson(researchText)) } catch {}
        } catch {}
      }
    } catch (e) {
  ...
  async function handleGenerateLetter() {
    ...
    try {
      ...
      const text = await callClaude('generateAppLetter', {
            ...
            <div className="card border-brand-blue border-2 bg-blue-50">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div>
                  ...
                  {/* Active extras reminder */}
                  {(personalNotes.trim() || resumeLink.trim()) && (
