...
import { getProfile, getLastBrief } from '../lib/storage'
...
export default function OutreachGenerator({ prospect }) {
  ...
  function extractSection(text, sectionKey) {
    const patterns = {
      connection: /##\s*[✉️🔗]\s*1\.\s*CONNECTION REQUEST[\s\S]*?(?=##|$)/i,
      firstDm:    /##\s*[💬]\s*2\.\s*FIRST DM[\s\S]*?(?=##|$)/i,
      followUp1:  /##\s*[🔁]\s*3\.\s*FOLLOW-UP 1[\s\S]*?(?=##|$)/i,
      followUp2:  /##\s*[🔁]\s*4\.\s*FOLLOW-UP 2[\s\S]*?(?=##|$)/i,
      followUp3:  /##\s*[🔁]\s*5\.\s*FOLLOW-UP 3[\s\S]*?(?=##|$)/i,
    }
  ...
  function copySection(key, text) {
    // Extract just the message text from the section
    const section = extractSection(result, key)
    if (section) {
      // Try to get just the message content
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {result && (
        <div className="card fade-in">
          ...
          {/* Usage notes (at the bottom of full result) */}
