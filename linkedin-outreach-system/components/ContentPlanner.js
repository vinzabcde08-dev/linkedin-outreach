  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const res = await fetch('/api/claude', {
        ...
        body: JSON.stringify({ feature: 'generateContent', data: { date: today }, profile }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Request failed')

      setRawResult(data.result)
      const parsed = parseIdeas(data.result)
      setIdeas(parsed)
      setExpandedIdea(0)
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h2 className="section-title">Daily Content Planner</h2>
          <p className="section-subtitle">
            Generate 5 LinkedIn content ideas with hooks, formats, and full scripts — tailored to your expertise and audience.
          </p>
        </div>
